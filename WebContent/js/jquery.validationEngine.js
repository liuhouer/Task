...
jQuery.fn.validationEngine = function(settings) {
	if ($.validationEngineLanguage) { // IS THERE A LANGUAGE LOCALISATION ?
		...
		allRules = {
			"required" : { // Add your regex rules here, you can take telephone
				// as an example
				"regex" : "none",
				"alertText" : "* 必填",
	...
	$("form").bind("submit", function(caller) { // ON FORM SUBMIT, CONTROL AJAX
		// FUNCTION IF SPECIFIED ON
		// DOCUMENT READY
	...
	var buildPrompt = function(caller, promptText) { // ERROR PROMPT CREATION
		// AND DISPLAY WHEN AN
		// ERROR OCCUR
	...
	var validateCall = function(caller, rules) { // EXECUTE VALIDATION
		...
		if (isError == true) {
			...
			if ($("input[name=" + prompt + "]").size() > 1
					&& callerType == "radio") { // Hack for radio group button,
				// the validation go the first
				// radio
				caller = $("input[name=" + prompt + "]:first");
			}
			($("." + prompt).size() == 0) ? buildPrompt(caller, promptText)
		...
		function _required(caller, rules) { // VALIDATE BLANK FIELD
			...
			if (callerType == "text" || callerType == "password"
				...
				if (!$(caller).val()||$(caller).val()==""||$(caller).val()==''||$(caller).val()==null) {
					isError = true;
			...
			if (callerType == "select-one") { // added by paul@kinetek.net for
				...
				callerName = $(caller).attr("name");
				if (!$("select[name=" + callerName + "]").val()||$("select[name=" + callerName + "]").val()==""||$("select[name=" + callerName + "]").val()==''||$("select[name=" + callerName + "]").val()==null) {
					isError = true;
					promptText += settings.allrules[rules[i]].alertText
							+ "<br />";
				}
			...
			if (callerType == "select-multiple") { // added by paul@kinetek.net
				...
				if (!$("#" + callerName).val()||$("#" + callerName).val()==""||$("#" + callerName).val()==''||$("#" + callerName).val()==null) {
					isError = true;
					promptText += settings.allrules[rules[i]].alertText
							+ "<br />";
				}
	...
	var submitValidation = function(caller) { // FORM SUBMIT VALIDATION
		...
		if (stopForm) { // GET IF THERE IS AN ERROR OR NOT FROM THIS VALIDATION
			// FUNCTIONS
